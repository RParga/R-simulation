...
for (iter in 1:tmax) {
  ...
  for(t in seq(1, (length(fuerza.reducida) -1), by = 2)){
    aux <- c(aux, (sum((fuerza.reducida[t])^2, (fuerza.reducida[t +1])^2))^(1/2))
  }
  respuesta <- rbind(respuesta, aux)
  tl <- paste(iter, "", sep="")
  while (nchar(tl) < digitos) {
    tl <- paste("0", tl, sep="")
  }
...
plot(y= promedios, x= p$m, main= "promedio de movimiento vs masa", xlab = "Masas", ylab= "Promedios")
graphics.off()
for(h in 1:tmax){
  png(paste("histograma ", h,  ".png", sep=""))
  hist(main = paste("histograma paso ", h), as.numeric(respuesta[h,]), xlab = "Distancia recorrida", ylab = "Frecuencia")
  graphics.off()
}